# MUDO-Groupware k6 Result - peak-load-500-run1 (p99 열 제거본, PPT용)

> 원본: [`peak-load-500-run1-summary.md`](peak-load-500-run1-summary.md)
> p99는 k6 실행 시 `summaryTrendStats`에 `p(99)`가 빠져 있어 실제로 측정되지 않은 값이라 열 자체를 제거했습니다.
> (threshold `p(99)<4000ms`는 내부적으로 통과 확인됨 — 정확한 수치만 미기록)
...
| Metric | avg(ms) | min(ms) | med(ms) | p90(ms) | p95(ms) | max(ms) |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
