...
본 서버는 하나의 Spring Boot 애플리케이션으로 배포되는 모듈형 모놀리스이며, 각 도메인 내부에는 하프 헥사고날 아키텍처를 적용한다. 도메인별 담당자는 자신의 도메인 패키지를 소유하고, 다른 도메인의 내부 구현을 직접 수정하거나 참조하지 않는다.
...
프로젝트는 레이어드 구조를 기본으로 하되, 필요한 도메인에 Port/Adapter를 적용하는 하이브리드 아키텍처를 사용한다.
