...
- 이 서비스는 학원(테넌트)마다 별도 EC2 애플리케이션 Task(jar 인스턴스)와 RDS 안의 별도 Database/User를 사용한다. 인프라 구조 상세는 [2026-08-03-academy-groupware-multi-tenant-aws-infra-guide.md](2026-08-03-academy-groupware-multi-tenant-aws-infra-guide.md) 참고.
- 하나의 배포 단위(Task) 안에서 실행되는 애플리케이션 코드는 항상 단일 학원의 데이터만 다룬다. 같은 스키마 안에 다른 학원의 데이터가 섞이는 경우가 없다.
- 따라서 도메인 코드는 조회·검증 조건에 `academyId`를 사용하지 않는다. 학원 간 데이터 격리는 애플리케이션 로직이 아니라 배포·DB 스키마 구조가 보장한다.
- 기존에 `academyId` 컬럼·검증 로직이 남아있는 도메인(FK가 실제로 걸린 `users`, `role` 등)은 즉시 제거 대상이 아니며, 담당 도메인 판단에 따라 별도로 정리한다. **신규 기능에는 academyId 검증 코드를 추가하지 않는다.**
