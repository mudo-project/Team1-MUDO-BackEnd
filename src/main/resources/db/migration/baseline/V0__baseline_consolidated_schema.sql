-- ============================================================================
-- Baseline consolidated schema (V0)
--
...
-- 생성보다 먼저 실행되던 기존 순서 역전 문제가 이 시점 이후로는 사라진다.
--
-- 이 베이스라인부터는 버전 0에서 다시 시작하며, 이후 신규 마이그레이션은
-- 다시 담당자별 버전(V{담당자번호}.{묶음번호}.{순번}) 규칙을 그대로 쓴다.
-- 즉 같은 유형의 순서 역전이 향후 새 마이그레이션에서 다시 발생할 수는
-- 있으나, `FlywayFreshDatabaseMigrationTest`(빈 DB 전체 마이그레이션 CI
-- 검증)가 머지 전에 잡아낸다. 걸리면 참조하는 테이블을 만드는 마이그레이션의
-- 버전(묶음번호.순번)을 낮춰 먼저 실행되도록 조정한다(docs/DATABASE.md의
-- "충돌 발생 시 대응" 참고).
--
