> 작성일: 2026-08-03
> 상태: 🚧 초기 스캐폴딩 완료 · 카테고리 제외 · 권한 정책 반영
...
- 수정은 `NOTICE:WRITE` 권한과 작성자 본인 검증을 함께 적용한다. 삭제는 작성자 본인만 가능하다. 고정/고정 해제는 `NOTICE:PIN` 권한으로 제한한다.
...
- [x] `./gradlew test` 통과.
- [ ] 카테고리 기능 필요 여부 팀 확인
- [x] 작성/수정은 `NOTICE:WRITE`, 고정/고정해제는 `NOTICE:PIN` 권한으로 반영. 삭제는 작성자 본인만 가능하게 유지
