...
- **테넌트 라우팅 조회(`TenantDirectoryController`/`TenantDirectoryQueryService`/`PlatformTenantDirectory`)도 예외다.** 프론트가 로그인 전에 학원 코드로 실제 API 호스트를 조회하는 공개 API(`/api/public/tenants/{code}`)라 `ConditionalOnProperty` 없이 모든 활성 테넌트 Task에서 항상 동작한다. `PLATFORM_TENANT_DIRECTORY_JSON`(`render_public_tenant_directory`)도 dashboard host 여부와 무관하게 모든 활성 테넌트에 주입된다. dashboard host 전용인 관리자 레지스트리(`PLATFORM_DASHBOARD_TENANT_REGISTRY_JSON`)와 혼동하지 않는다 — 두 값은 별개의 용도다.
