...
- **참여자·발신자 이름 조회**: `ChatMemberDirectoryPort`(application/port)로 추상화. User 도메인 모듈이 아직 없어, approval의 `UserNameEntity`·notice의 `UserInfoEntity`와 동일한 성격의 임시 shim(`infrastructure/persistence/UserInfoEntity`)으로 구현했다. User 모듈이 생기면 세 모듈 모두 정식 구현으로 교체해야 한다.
