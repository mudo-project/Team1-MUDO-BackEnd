...
- API 응답이나 동작에는 영향 없습니다(academyId는 원래도 응답에 노출되지 않던 내부 값입니다).

자세한 내용은 [REVISION.md](REVISION.md)를 참고해주세요.

## 2026-08-11 · 완료된 업무지시 카드 수정/삭제 제한

- 담당자 중 1명이라도 완료 처리한 업무지시 카드는 더 이상 수정하거나 삭제할 수 없습니다.
- 지금까지는 완료 기록이 남아있어도 카드 내용을 바꾸거나 카드 자체를 지울 수 있었습니다.

자세한 내용은 [REVISION.md](REVISION.md)를 참고해주세요.

## 2026-08-10 · 채팅방 academy_id 제거

- 채팅방(`ChatRoom`)이 더 이상 학원 단위로 스코프되지 않습니다 — 다른 학원 소속 사용자끼리도 채팅방을 만들 수 있습니다.
- 채팅방 생성 시 "다른 학원 소속 사용자는 초대할 수 없다"는 제한(`CROSS_ACADEMY_INVITE`, `MESSENGER_400_8`)이 없어졌습니다.
- 파일 다운로드 URL 조회는 여전히 요청자 본인의 학원 기준으로 스코프됩니다(채팅방이 아니라 파일 자체의 권한 문제라 별개입니다).
