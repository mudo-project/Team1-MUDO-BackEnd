...
| `colorCriterion` | Enum | true | 배경색을 어떤 기준으로 칠할지. `CLASSROOM`/`TEACHER` 중 하나 |
| `colorMap` | String(JSON) | true | `colorCriterion` 기준값(강의실 코드/강사명) → 6자리 16진수 색상의 JSON 객체. 예: `{"601":"FFCC00","602":"00AACC"}` |
...
- 배경색은 `colorCriterion`으로 지정한 기준(강의실 코드/강사명)의 값을 키로 `colorMap`에서 조회해 적용한다. `colorMap`에 없는 값이거나 해당 슬롯의 기준 필드가 비어 있으면 흰색(기본값)으로 표시한다. 백엔드가 팔레트를 계산하지 않고 프론트가 지정한 값을 그대로 쓴다. 학년(`grade`)은 색 기준으로 쓰지 않는다 — 화면 설계상 색 기준은 강의실/강사만 지원하기로 확정됐다.
