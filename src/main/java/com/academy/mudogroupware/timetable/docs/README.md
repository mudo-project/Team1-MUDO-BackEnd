...
학원의 시간표를 학기/특강 기간 단위(시간표 세트)로 관리한다. 이번 범위는 **시간표 세트 관리**(생성/조회/수정/삭제)까지다. 세트 안에 실제로 들어가는 개별 수업(요일/시간/강의실/강사/과목)인 "수업 슬롯" CRUD와 겹침 검사, 회차 단위 수정/삭제(해당 회차만/현재부터 전체/전체)는 별도 범위(후속 이슈)다.
...
- 세트 삭제 시 `timetable_set_classroom`은 `ON DELETE CASCADE`로 함께 삭제된다. 이후 수업 슬롯 테이블(`timetable_slot`)도 같은 방식으로 연결될 예정이라, 세트 삭제만으로 그 안의 모든 데이터가 정리된다.
- 마이그레이션 담당자번호는 `be5` (`V5.1.3`, `V5.1.4`).

## 세부 문서

- [BUSINESS_RULES.md](BUSINESS_RULES.md) — 도메인 정책과 접근 권한, 검증 규칙
- [TIMETABLE_API.md](TIMETABLE_API.md) — 엔드포인트별 요청·응답·에러 코드
