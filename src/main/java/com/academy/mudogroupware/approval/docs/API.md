...
| `409 Conflict` | `APPROVAL_409_10` | 다른 요청이 먼저 이 결재를 처리했습니다. 새로고침 후 다시 시도해주세요. | 같은 결재 문서를 동시에 처리하는 요청이 겹쳐 낙관적 락 충돌이 발생한 경우 |
...
| `409 Conflict` | `APPROVAL_409_10` | 다른 요청이 먼저 이 결재를 처리했습니다. 새로고침 후 다시 시도해주세요. | 취소 요청과 동시에 다른 요청이 같은 문서를 먼저 처리해 낙관적 락 충돌이 발생한 경우 |
...
| `APPROVAL_409_10` | 409 | 다른 요청이 먼저 이 결재를 처리했습니다. 새로고침 후 다시 시도해주세요. |
