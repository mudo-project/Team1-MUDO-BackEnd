...
- Google 계정 테이블과 FK를 두지 않는다. 공유파일은 현재 배포 단위의 루트를 관리하고, Google 모듈에는 접근 토큰만 요청한다. `GetGoogleAccessTokenUseCase.getAccessToken()`은 파라미터가 없으며 `academyId`를 요구하지 않는다.
