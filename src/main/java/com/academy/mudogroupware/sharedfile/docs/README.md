...
- 학원별 시스템 루트 폴더 이름은 `이음 그룹웨어 - 공유파일`로 고정한다.
...
- 애플리케이션 DB에는 학원별 시스템 루트의 Google 폴더 ID와 생성·복구 상태만 저장한다.
...
`shared_file_root`는 학원별 현재 시스템 루트 하나만 관리한다. 파일·폴더 목록을 저장하는 테이블이 아니다.

| 컬럼 | 규칙 | 의미 |
| --- | --- | --- |
| `shared_file_root_id` | PK | 시스템 루트 설정 식별자 |
| `academy_id` | FK, UNIQUE | 학원당 설정 한 건 |
...
- Google 계정 테이블과 FK를 두지 않는다. 공유파일은 학원 기준으로 현재 루트를 관리하고, Google 모듈에는 접근 토큰만 요청한다.
