...
| `409 Conflict` | `WORKSPACE_409_1` | 활성 이름 unique 제약 충돌 후 1회 재시도도 실패한 경우 |
...
- 이름의 앞뒤 공백을 제거한 뒤 활성 워크스페이스 이름이 이미 있으면 가장 작은 사용 가능 번호를 붙인다.

```text
업무현황 → 업무현황 (1) → 업무현황 (2)
```

- 자동 접미사가 붙으면 전체 이름이 100자를 넘지 않도록 원래 이름을 줄인다.
- 동시 생성으로 DB unique 제약이 충돌하면 새 트랜잭션에서 이름을 다시 계산해 한 번 재시도한다.
