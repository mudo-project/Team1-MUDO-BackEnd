...
public class Workspace {
  ...
  private Workspace(Long id, String name, Long createdBy, Set<Long> memberIds) {
    this.id = id;
    this.name = name;
  ...
  public static Workspace create(String name, Long creatorId, Set<Long> additionalMemberIds) {
    Set<Long> memberIds = new LinkedHashSet<>(additionalMemberIds);
    memberIds.add(creatorId);
    return new Workspace(null, name, creatorId, memberIds);
  }

  public static Workspace restore(Long id, String name, Long createdBy, Set<Long> memberIds) {
    return new Workspace(id, name, createdBy, memberIds);
  }

  public Workspace rename(String newName) {
    return new Workspace(id, newName.trim(), createdBy, memberIds);
  }

  public Workspace addMembers(Set<Long> candidateIds) {
    Set<Long> updated = new LinkedHashSet<>(memberIds);
    updated.addAll(candidateIds);
    return new Workspace(id, name, createdBy, updated);
  }
  ...
  public Workspace recover(String finalName) {
    return new Workspace(id, finalName, createdBy, memberIds);
  }
}
