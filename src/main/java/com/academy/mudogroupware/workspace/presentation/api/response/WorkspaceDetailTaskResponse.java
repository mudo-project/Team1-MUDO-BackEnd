...
public record WorkspaceDetailTaskResponse(
    ...
    @Schema(description = "업무 제목", example = "8월 원생 청구서 발송") String title,
    @Schema(description = "업무 상태") TaskStatus status,
    @Schema(description = "생성자") WorkspaceDetailMemberResponse creator,
    @Schema(description = "기한. 반복 업무는 항상 null", example = "2026-08-07") LocalDate dueAt,
  ...
  public static WorkspaceDetailTaskResponse from(WorkspaceTaskItem item) {
    return new WorkspaceDetailTaskResponse(
        item.taskId(),
        item.title(),
        item.status(),
        WorkspaceDetailMemberResponse.from(item.creator()),
        item.dueAt(),
        item.completedCommentCount(),
        item.commentCount());
  }
}
