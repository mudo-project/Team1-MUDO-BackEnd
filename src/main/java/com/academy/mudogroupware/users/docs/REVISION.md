...
로그인 응답(`LoginResponse.permissions`)엔 최종 유효 권한 목록이 있지만, 토큰 재발급(`POST /api/auth/refresh`) 응답엔 없다. 관리자가 역할의 권한을 바꾸면 요청마다 라이브로 재계산되는 인가 자체는 즉시 반영되지만, 프론트가 화면에 그리는 "내가 뭘 할 수 있는지" 캐시는 재로그인 전까지 갱신할 방법이 없었다.

### 확정된 정책

- `GET /api/users/me/permissions` 신설. 로그인만 되면 누구나 호출 가능(특정 권한 요구 없음).
- 새 도메인 로직을 추가하지 않고 기존 `EffectivePermissionResolver`(로그인에서 이미 쓰는 컴포넌트)를 그대로 재사용한다. `AuthUser`가 이미 갖고 있는 `roleId`/`accountType`/`adminScope`를 그대로 넘겨서 계산하므로 추가 DB 조회가 없다.
- 응답 필드명은 `LoginResponse.permissions`와 동일하게 맞췄다(`{"permissions": [...]}`).

### 로컬 e2e 검증

- ACADEMY 관리자 계정으로 로그인 후 `GET /api/users/me/permissions` 호출 → 로그인 응답의 `permissions`와 정확히 동일한 값(`["ACCOUNT:MANAGE", "ROLE:MANAGE", "ACADEMY:OWNER"]`) 확인.
- 토큰 없이 호출 → 401 확인.
