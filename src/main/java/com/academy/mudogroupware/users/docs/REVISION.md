...
- 이메일 자동 발송, 원장 신청 시 username 중복 확인, 사업자등록증 검증(OCR·국세청 진위확인 API) — 기존부터 이어져 온 후속 작업, 이번에도 미룸.
