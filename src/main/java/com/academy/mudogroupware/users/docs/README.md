...
- `POST /api/academy-applications`(신청 접수)는 계정 없는 학원이 호출하므로 `SecurityConfig`에서 `permitAll`이다 — 같은 리소스의 나머지 4개 엔드포인트(GET·approve·reject)는 여전히 `PLATFORM:SUPER_ADMIN` 필요.
