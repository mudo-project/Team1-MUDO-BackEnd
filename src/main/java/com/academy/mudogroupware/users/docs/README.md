...
- 계정 발급 체계 3단계(학원 관리자의 직원 계정 발급, `POST /api/users`)까지 완료됐다. 임시 비밀번호 생성 로직을 담당하는 `AccountIssuer` 협력 객체가 `application/service/support`에 있다. (2026-08-12) `AccountIssuer`는 임시 비밀번호(`temporaryPassword`)를 응답에 직접 내려준다 — 이전에는 `PasswordSetupLinkBuilder`로 아이디·임시 비밀번호를 쿼리스트링에 실은 링크를 반환했으나, 그 방식이 브라우저 히스토리·서버 접근 로그·`Referer` 헤더로 노출될 수 있어 폐기하고 평문 응답 필드로 되돌렸다(`PasswordSetupLinkBuilder`/`app.frontend-url` 설정 모두 삭제). `mustChangePw`는 로그인 흐름에서 다른 API를 막는 강제 로직으로 쓰지 않고, 로그인 응답과 JWT 클레임에 담아 프론트에 신호만 주는 플래그로 쓴다(강제 로직을 안 만들기로 한 배경은 `REVISION.md` 참고). 이메일 발송 등은 아직 없다(후속 작업, 상세 배경은 `REVISION.md` 참고).
...
- `GET /api/users/{userId}`(관리자용 구성원 상세조회)는 `ACCOUNT:MANAGE` 권한이 필요하고, `GetUserDetailService`가 `GetMyProfileUseCase`/`GetMemberDetailUseCase` 둘 다 구현하는 방식으로 내 정보 조회와 응답 형태를 그대로 재사용한다. 대상이 없거나 학원 관리자 계정이면 구성원 정보 수정 API와 동일하게 `404 USER_404_1`로 응답해 존재 여부를 숨긴다.
- `PATCH /api/users/{userId}`(관리자용 구성원 정보 수정)는 같은 방식으로 `UpdateUserProfileService`가 `UpdateMyProfileUseCase`/`UpdateMemberProfileUseCase` 둘 다 구현한다. 본인 수정(`PATCH /api/users/me`)과 달리 `name`/`joinedAt`도 바꿀 수 있다. (2026-08-12) 프론트 요청으로 별도 API였던 `PATCH /api/users/{userId}/role`(사용자 역할 변경)을 이 API로 병합했다 — 요청 바디에 선택 필드 `roleId`를 추가하고, `UpdateUserProfileService`가 `RoleRepository`도 의존하도록 확장했다. `roleId`가 오면 역할 존재를 먼저 검증(`RoleNotFoundException`, `404 USER_404_2`)한 뒤에야 나머지 필드를 갱신한다 — 역할이 잘못됐는데 이름 등 다른 필드만 반영되는 부분 실패를 막기 위함이다. 옛 `ChangeUserRoleCommand`/`ChangeUserRoleUseCase`/`ChangeUserRoleService`/`ChangeUserRoleRequest`는 전부 삭제했다(다른 어디서도 참조하지 않음, `UserRepository.changeRole`/`UserRepositoryImpl.changeRole`은 그대로 재사용).
...
- `PATCH /api/users/me/password`는 `POST /api/users/password-setup`(최초 1회 설정 전용)과 별개다 — 둘 다 인증이 필요하지만, 후자는 "이미 설정 완료" 재요청과 그 외 실패를 의도적으로 구분하지 않는 마스킹된 응답(`USER_400_2`)을 유지하고, 전자는 현재 비밀번호가 틀리면 구체적인 오류 메시지(`USER_400_3`)를 반환한다.
- (2026-08-12) 계정 생성(`POST /api/users`)은 `username`/`name`/`roleId`만 받는다 — `phone`/`email`은 이 API에서 아예 받지 않고(요청 바디에 넣으면 `400`으로 거절), 최초 비밀번호 설정(`POST /api/users/password-setup`)에서 새 비밀번호와 함께 필수로 등록하도록 강제한다. 이 엔드포인트가 인증 필요로 바뀐 것도 같은 결정의 일부다 — `username`/임시 비밀번호를 요청 바디로 주고받지 않고 JWT로 신원을 확인한다.
- (2026-08-12) `ACCOUNT:MANAGE`와 `ROLE:MANAGE`는 원칙적으로 서로 다른 리소스라 별도 권한 코드로 남아있다. 다만 계정 발급 시 `roleId`를 선택해야 하는 실무 필요 때문에, 역할 목록/상세 조회(`GET /api/roles`, `GET /api/roles/{roleId}`, 읽기 전용)만 예외적으로 `ACCOUNT:MANAGE` 권한으로도 호출할 수 있게 열어뒀다. 역할 생성·수정·삭제·권한 조립(쓰기 작업)은 여전히 `ROLE:MANAGE` 전용이다 — "관련 행위는 하나의 코드로 묶는다" 원칙을 계정↔역할처럼 다른 리소스 간에 그대로 확장하지 않고, 실제로 필요한 읽기 경로만 최소로 열었다.
...
- **`GET /api/users/members` `status` 필터(2026-08-12)**: 응답에는 이미 구성원별 `status`가 내려가고 있었는데 조회 쪽에는 필터링 파라미터가 없어서, 프론트가 재직/휴직/퇴사 탭을 서버 페이지네이션과 함께 구현할 방법이 없었다. `keyword`/`roleId`와 동일한 방식(인메모리 스트림 필터)으로 `status`(`ACTIVE`/`RESIGNED`/`INACTIVE`) 파라미터를 추가했다 — 셋 다 AND 조건으로 함께 적용된 뒤 정렬·페이지네이션이 이루어진다.
