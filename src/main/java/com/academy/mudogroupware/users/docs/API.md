...
- 사업자등록번호·사업자등록증 파일은 이번 스코프에서 받지 않습니다(`businessNo`/`businessLicenseFileId`는 항상 `null`로 생성됨). 소유권 검증은 SUPER ADMIN이 승인 단계에서 수동으로 확인합니다.
- `requestedLoginId` 중복 확인은 하지 않습니다 — 실제 충돌은 승인 시점 계정 생성에서 DB UNIQUE 제약으로 잡힙니다.
- rate limit 등 악의적 공격 방어 로직은 이번 스코프에 포함되지 않습니다.
