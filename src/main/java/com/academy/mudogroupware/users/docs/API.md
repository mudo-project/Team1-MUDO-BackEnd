...
| `404` | `USER_404_1` | 리프레시 토큰의 사용자 정보를 찾을 수 없음, 사용자 역할 변경 또는 구성원 상세 조회(15-3번 항목) 시 대상 계정이 존재하지 않거나 다른 학원 소속이거나 학원 관리자 계정임, 또는 내 정보 조회(15번 항목) 시 JWT의 userId에 해당하는 계정을 찾을 수 없음(계정 삭제 등) |
...
| `409` | `USER_409_7` | 내 정보 수정(15-1번 항목) 시 이미 사용 중인 이메일로 변경 시도 |
