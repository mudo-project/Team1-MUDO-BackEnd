...
`GET /api/roles`
권한: `ROLE:MANAGE` 또는 `ACCOUNT:MANAGE` 필요(2026-08-12)
...
- (2026-08-12) 계정 발급(13번 항목) 시 `roleId`를 골라야 하므로, `ACCOUNT:MANAGE` 권한만 있는 계정도 이 조회 API를 호출할 수 있습니다. 역할 생성·수정·삭제·권한 조립(쓰기 작업)은 여전히 `ROLE:MANAGE` 전용입니다 — 계정 생성 권한과 역할 자체를 바꾸는 권한은 서로 다른 리소스로 취급합니다.
...
- (2026-08-12) 역할 목록 조회와 동일하게 `ACCOUNT:MANAGE` 권한만 있는 계정도 호출할 수 있습니다.
...
- `userId`가 존재하지 않거나 `accountType != MEMBER`(학원 관리자 계정)이면 전부 동일하게 `404 USER_404_1`로 응답합니다 — 관리자 계정 여부가 노출되지 않도록 하기 위함입니다.
...
- (2026-08-12) `phone`/`email`은 이 API에서 아예 받지 않습니다 — 요청 바디에 넣어도 알려지지 않은 필드로 취급되어 `400 COMMON_400_1`로 거절됩니다. 관리자가 직원 전체의 연락처를 대신 입력·관리하지 않고, 본인이 최초 비밀번호 설정(14번 항목)에서 직접 등록하도록 강제합니다.
- (2026-08-12) 응답이 `passwordSetupLink`(URL)에서 `temporaryPassword`(평문 임시 비밀번호)로 되돌아왔습니다 — 링크의 쿼리스트링에 아이디·임시 비밀번호를 실어 보내는 방식은 브라우저 히스토리, 서버·프록시 접근 로그, 같은 페이지에서 로드하는 외부 리소스로의 `Referer` 헤더에 그대로 노출될 수 있어 폐기했습니다. `temporaryPassword`는 이 호출 한 번에만 내려가며 서버에 별도로 저장되지 않습니다. 학원 관리자가 아이디·임시 비밀번호를 직원에게 직접 전달해야 합니다(카카오톡/문자 등) — 이메일 등 자동 발송은 아직 없습니다(후속 작업). 직원은 이 아이디·임시 비밀번호로 평소처럼 로그인(1번 항목)한 뒤, 로그인 응답의 `mustChangePw=true`를 신호로 최초 비밀번호 설정(14번 항목)을 진행합니다.
...
권한: 없음 (단, 로그인은 필요 — 임시 비밀번호로 로그인한 본인만 호출 가능)
...
| `email` | String | true | 이메일, 최대 100자, 형식 검증(`@Email`) |
...
- (2026-08-12) 인증이 필요한 엔드포인트로 바뀌었습니다 — 대상 계정은 `Authorization: Bearer {accessToken}`의 JWT에서 식별하며, 요청 바디에 `username`/임시 비밀번호를 담지 않습니다. 직원 계정 발급(13번 항목)으로 받은 아이디·임시 비밀번호로 먼저 로그인(1번 항목)해야 이 API를 호출할 수 있습니다.
- 검증 순서: JWT의 `userId`로 계정을 다시 조회해 `must_change_pw == true`인지 확인(최신 DB 상태 기준, 로그인 시점 JWT의 `mustChangePw` 클레임을 그대로 믿지 않음) → 통과하면 비밀번호·이메일·전화번호를 한 트랜잭션으로 교체. 이미 설정을 마친 계정이면 `400 USER_400_2`로 응답합니다(구체적인 사유는 구분하지 않고 동일하게 응답 — 재요청 시 "이미 처리됨"과 그 외 실패를 구분하지 않는 것은 의도된 동작입니다).
- `email`이 다른 계정과 중복되면 `409 USER_409_7`로 거절합니다(비밀번호·전화번호는 반영되지 않음 — 단일 UPDATE 문으로 처리되어 부분 반영이 구조적으로 불가능).
- 성공하면 `password`/`phone`/`email`이 새 값으로 교체되고 `must_change_pw`가 `false`로 바뀝니다. 그 순간부터 옛 임시 비밀번호는 더 이상 로그인에 쓸 수 없고, `must_change_pw`가 이미 `false`라 이 계정에 대해 이 엔드포인트 자체가 다시는 통과하지 않습니다(1회성 보장에 별도 만료시간을 두지 않음).
...
- 최초 비밀번호 설정(14번 항목)도 이제 인증이 필요한 엔드포인트지만, "이미 설정 완료" 재요청과 그 외 실패를 의도적으로 구분하지 않는 마스킹된 응답을 유지합니다(2026-08-12 재검토 후에도 유지하기로 결정 — 재요청 시 혼동보다 일관된 응답을 우선). 반면 이 API(15-2번 항목)는 현재 비밀번호 불일치라는 구체적인 사유를 그대로 반환합니다.
...
- `userId`가 존재하지 않거나 `accountType != MEMBER`(학원 관리자 계정)이면 전부 동일하게 `404 USER_404_1`로 응답합니다 — 관리자 계정 여부가 노출되지 않도록 하기 위함입니다(사용자 역할 변경 API와 동일한 정책).
...
| `404` | `USER_404_1` | 리프레시 토큰의 사용자 정보를 찾을 수 없음, 사용자 역할 변경·구성원 상세 조회(15-3번 항목)·구성원 정보 수정(15-4번 항목)·구성원 재직 상태 변경(15-5번 항목) 시 대상 계정이 존재하지 않거나 학원 관리자 계정임, 또는 내 정보 조회(15번 항목) 시 JWT의 userId에 해당하는 계정을 찾을 수 없음(계정 삭제 등) |
| `409` | `USER_409_1` | 이미 존재하는 역할 이름 |
| `409` | `USER_409_2` | 배정된 구성원이 있는 역할을 삭제하려 시도 |
| `400` | `USER_400_1` | 존재하지 않는 권한 코드로 역할 권한 조립 시도 |
| `400` | `USER_400_2` | 최초 비밀번호 설정(14번 항목) 시 이미 설정을 완료한 계정으로 재요청(사유를 구분하지 않고 공통 응답) |
| `404` | `USER_404_2` | 역할이 존재하지 않음 |
| `409` | `USER_409_6` | 이미 사용 중인 아이디로 직원 계정 발급 시도 |
| `409` | `USER_409_7` | 내 정보 수정(15-1번 항목), 구성원 정보 수정(15-4번 항목) 또는 최초 비밀번호 설정(14번 항목) 시 이미 사용 중인 이메일로 변경 시도 |
