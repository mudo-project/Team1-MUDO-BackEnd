...
- `GET /api/google/connections/status`를 추가했다. `NOT_CONNECTED` / `CONNECTED` / `EXPIRING` / `EXPIRED` / `FAILED` 중 하나를 `data.status`로 반환하며, Google API를 호출하거나 토큰 갱신을 시도하지 않는다.
...
- `GetGoogleAccountConnectionServiceTest`: `connectedByUserName` 조합을 검증.
...
- 기능 단위 테스트 31건 통과. 전체 테스트와 추가 `@SpringBootTest` 실행은 CI 시간이 길어 요청에 따라 생략했다.
