...
솔라피(SOLAPI) REST API로 실제 발송을 구현했다(2026-08-10). 실제 발송 테스트로 정상 접수(`statusCode: "2000"`)와 실제 수신까지 확인했다. 학생별 발송 시도를 `attendance_message_send_record`에 저장해, 같은 (강의, 학생, 출결 날짜)로 이미 발송에 성공했으면 재요청이 와도 SOLAPI를 다시 부르지 않는다(2026-08-14, 이슈 #354). 발송 이력 자동 재시도 스케줄링과 과금 정책은 아직 없다.
