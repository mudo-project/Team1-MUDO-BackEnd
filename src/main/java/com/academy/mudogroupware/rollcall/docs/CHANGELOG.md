...
## 2026-08-14 - 출결 SMS 발송 시 플랜 한도 체크 추가

- `SendAttendanceMessagesService.send()`가 학생별 발송 루프를 돌기 전, 이번 달 SMS 사용량이 플랜 한도(무료 150건/유료 10,000건)를 넘으면 배치 전체를 429로 차단하도록 변경했다. 배치 도중 한도를 소진하면 그 이후 학생은 실제 발송 없이 건너뛴다.
