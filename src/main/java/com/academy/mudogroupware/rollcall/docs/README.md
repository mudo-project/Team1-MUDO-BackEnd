...
- **문자 발송**: 선택한 학생들에게 솔라피(SOLAPI) API로 실제 SMS를 발송하고, 학생별 성공/실패를 반환한다. 발송 직전에 템플릿 변수 `{학생명}`/`{studentName}`, `{강의명}`/`{lectureName}`, `{날짜}`/`{date}`를 실제 값으로 치환한다.
...
- 문자 템플릿 본문의 `{학생명}`/`{studentName}`, `{강의명}`/`{lectureName}`, `{날짜}`/`{date}`는 실제 SMS 발송 직전에만 치환된다. 목록 조회에서는 저장된 템플릿 원문을 그대로 보여준다.
