...
- 최초 작성일: 2026-08-06
- 상태: 일정 생성·목록/일별 조회 API 확정, 상세조회·수정·삭제는 추후 이슈로 진행
...
- 사용 중인 예외: `CalendarTitleRequiredException`(400), `InvalidCalendarPeriodException`(400), `InvalidCalendarQueryException`(400, 목록/일별 조회 시 `date`/`yearMonth`를 둘 다 지정하거나 둘 다 생략한 경우), `CalendarEventNotFoundException`(404, `addContext`로 `eventId` 기록).
