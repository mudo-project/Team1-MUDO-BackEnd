...
- 작성/수정/삭제는 기능명세서상 "대표와 대표가 허용한 권한을 가져야" 가능하다고 되어 있으나, `users.role` 값 체계가 아직 자유 텍스트라 실제 권한 검사는 만들지 않았다. `notice` 도메인과 동일하게 로그인 여부만 검사하고 TODO로 남긴다(`users.role` 체계 확정 후 반영).
...
- **API 단위 진행**: 6개 엔드포인트를 한 번에 다 만들지 않고, API 하나씩(예: 일정 생성 → 일정 조회 → ...) 구현 → 이슈(`feature_request.yml` 형식) + PR(`pull_request_template.md` 형식) 문서 작성 → develop 머지 → 다음 API용 브랜치로 진행한다. 단, `git push`/PR 생성/머지는 AGENTS.md 절대 규칙("GitHub 직접 push 금지")에 따라 작업자가 직접 수행하고, 텍스트로 준비만 한다. 도메인 모델·예외·엔티티·영속성처럼 모든 API가 공유하는 기반 코드는 첫 API(일정 생성) 이슈/PR에 포함한다.
