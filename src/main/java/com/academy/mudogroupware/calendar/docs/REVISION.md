...
- `GetCalendarEventsUseCase`/`GetCalendarEventsService`는 조회 기간(`from`, `to`)만 전달하며 현재 테넌트 DB를 조회합니다.
...
- `DeleteCalendarEventCommand(eventId)`로 삭제할 일정을 지정합니다.
- `DeleteCalendarEventService`는 `findById` 결과가 없으면 `CalendarEventNotFoundException`(`CALENDAR_404_1`)을 던집니다.
...
- `UpdateCalendarEventService`는 `findById` 결과가 없으면 `CalendarEventNotFoundException`(`CALENDAR_404_1`)을 던집니다.
...
- 조회는 Command 객체 없이 `from`, `to`를 UseCase에 전달합니다. `memo` 도메인의 조회 패턴(Domain Model을 그대로 반환, 별도 View 불필요)을 따랐습니다.
...
- 조회 대상은 현재 테넌트 DB로 한정하며, 다른 학원의 데이터는 별도 DB에 저장됩니다.
...
- `POST /api/calendars` 엔드포인트는 `title`, `content`, `eventStartAt`, `eventEndAt`, `allDay`, `color`를 받고, `createdBy`는 `AuthUser`에서 채웁니다.
