...
@Component public class TraceIdFilter extends OncePerRequestFilter { public static final String TRACE_ID="traceId",TRACE_ID_HEADER="X-Trace-Id";
 protected void doFilterInternal(HttpServletRequest q,HttpServletResponse s,FilterChain c)throws ServletException,IOException{String t=UUID.randomUUID().toString().substring(0,8);try{MDC.put(TRACE_ID,t);MDC.put("method",q.getMethod());MDC.put("requestURI",q.getRequestURI());MDC.put("remoteAddr",q.getRemoteAddr());s.setHeader(TRACE_ID_HEADER,t);c.doFilter(q,s);}finally{MDC.clear();}}
}
