    ...
    if (StringUtils.hasText(t)) {
      try {
        SecurityContextHolder.getContext()
            .setAuthentication(converter.toAuthentication(provider.parseAccessToken(t)));
      } catch (AuthException e) {
        q.setAttribute("authErrorCode", e.getErrorCode());
      }
    }
    c.doFilter(q, s);
  }

  private String resolve(HttpServletRequest q) {
    String h = q.getHeader(HttpHeaders.AUTHORIZATION);
    if (StringUtils.hasText(h) && h.startsWith(PREFIX)) {
      String t = h.substring(PREFIX.length());
      if (StringUtils.hasText(t) && !"null".equals(t) && !"undefined".equals(t)) return t;
    }
