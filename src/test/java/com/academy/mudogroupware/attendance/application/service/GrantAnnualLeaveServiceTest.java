...
class GrantAnnualLeaveServiceTest {
    ...
    void grantsCurrentAnnualPeriodWhenMissing() {
        ...
        when(leaveGrantRepository.saveIfAbsent(any(LeaveGrant.class))).thenReturn(true);

        int count = service.grantAnnualLeave(now);

        assertEquals(1, count);
        ArgumentCaptor<LeaveGrant> captor = ArgumentCaptor.forClass(LeaveGrant.class);
        verify(leaveGrantRepository).saveIfAbsent(captor.capture());
    ...
    void skipsExistingCurrentPeriod() {
        ...
        when(leaveGrantRepository.saveIfAbsent(any(LeaveGrant.class))).thenReturn(false);

        int count = service.grantAnnualLeave(now);

        assertEquals(0, count);
        verify(leaveGrantRepository).saveIfAbsent(any(LeaveGrant.class));
    }
}
