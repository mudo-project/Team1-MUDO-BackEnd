...
class MyAttendanceDashboardQueryServiceTest {
    ...
    void composesExistingQueryResultsWithoutRecalculatingThem() {
        ...
        when(monthlyUseCase.getMonthly(userId, 2026, 8)).thenReturn(monthly);
        when(todayUseCase.getToday(userId)).thenReturn(today);
        when(leaveUseCase.getSummary(userId)).thenReturn(leave);
        when(employmentUseCase.getSummary(userId)).thenReturn(employment);
        var service = new MyAttendanceDashboardQueryService(
                monthlyUseCase, todayUseCase, leaveUseCase, employmentUseCase);

        var result = service.getDashboard(userId, 2026, 8);
        ...
        verify(monthlyUseCase).getMonthly(userId, 2026, 8);
        verify(todayUseCase).getToday(userId);
        verify(leaveUseCase).getSummary(userId);
        verify(employmentUseCase).getSummary(userId);
    }
}
